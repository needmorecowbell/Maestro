...
import { EncoreSettings } from './EncoreManager';
import { EncoreFeatureCard, DirectorNotesSettings } from './Settings';
...
interface SettingsModalProps {
	...
	encoreRegistry?: {
		encores: import('../../shared/encore-types').LoadedEncore[];
		loading: boolean;
					...
					{activeTab === 'encore' && (
						<div className="space-y-6">
							...
							<EncoreFeatureCard
								...
								enabled={encoreFeatures.directorNotes}
								onToggle={() =>
									setEncoreFeatures({
										...encoreFeatures,
							...
							>
								<DirectorNotesSettings
									theme={theme}
									settings={directorNotesSettings}
									onSettingsChange={setDirectorNotesSettings}
								/>
							</EncoreFeatureCard>

							{/* Installable encores — each gets its own card */}
							{props.encoreRegistry?.encores.map((encore) => {
								const enabled = encore.state === 'active' || encore.state === 'loaded';
								return (
									<EncoreFeatureCard
										key={encore.manifest.id}
										theme={theme}
										icon={<Puzzle className="w-5 h-5" />}
										name={encore.manifest.name}
										description={encore.manifest.description}
										version={encore.manifest.version}
										author={encore.manifest.author}
										permissions={encore.manifest.permissions}
										enabled={enabled}
										onToggle={async () => {
											if (enabled) {
												await props.encoreRegistry?.disableEncore(encore.manifest.id);
											} else {
												await props.encoreRegistry?.enableEncore(encore.manifest.id);
											}
										}}
									>
										<div
											...
											style={{ borderColor: theme.colors.border }}
										>
											<EncoreSettings encore={encore} theme={theme} />
										</div>
									</EncoreFeatureCard>
								);
							})}

...
describe('useStats', () => {
  ...
  describe('initialization and data fetching', () => {
    ...
    it('should set error state when fetch fails', async () => {
      vi.mocked(window.maestro.stats.getAggregation).mockRejectedValue(
  ...
  describe('time range options', () => {
    it.each([
      ...
      'year' as const,
      'all' as const,
    ])('should support %s time range', async (range) => {
      const mockData: StatsAggregation = {
        totalQueries: 10,
        totalDuration: 360000,
        avgDuration: 36000,
